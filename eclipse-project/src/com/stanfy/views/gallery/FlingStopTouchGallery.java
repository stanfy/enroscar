...
public class FlingStopTouchGallery extends Gallery implements Runnable {
  ...
  public FlingStopTouchGallery(final Context context) {
    this(context, null);
  }

  public FlingStopTouchGallery(final Context context, final AttributeSet attrs) {
    this(context, attrs, android.R.attr.galleryStyle);
  }

  public FlingStopTouchGallery(final Context context, final AttributeSet attrs, final int defStyle) {
  ...
  public boolean onTouchEvent(final MotionEvent event) {
    ...
    if (action == MotionEvent.ACTION_UP && !flingMode && lastSelectedPosition != currentPosition) {
      ...
      if (DEBUG) { Log.d(VIEW_LOG_TAG, "lastSelectedPosition=" + lastSelectedPosition); }
